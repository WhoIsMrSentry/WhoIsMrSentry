...
def _format_html(track: Dict[str, Any], playlist: Dict[str, Any], payload: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f'  <p style="margin:0 0 8px 0;"><strong>{status_label}:</strong> '
        f'<a href="{track_url}" style="color:#f5f5f5;">{track_name}</a> — {artists}</p>\n'
        f'  <p style="margin:0 0 12px 0;"><strong>Playlist:</strong> '
        f'<a href="{playlist_url}" style="color:#f5f5f5;">{playlist_name}</a></p>\n'
        ...
        f'<img src="{track_cover}" alt="{track_name} cover art" height="160" '
        'style="border:1px solid #52000f;border-radius:12px;" /></a>\n'
        f'    <a href="{playlist_url}" title="Open {playlist_name}">'
        f'<img src="{playlist_cover}" alt="{playlist_name} artwork" height="160" '
        'style="border:1px solid #52000f;border-radius:12px;" /></a>\n'
        '  </div>\n'
        f'  <p style="margin-top:12px;font-size:12px;"><em>Updated {timestamp}</em></p>\n'
        '</div>'
    )
